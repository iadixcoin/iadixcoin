...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("iadixcoin"))
        return false;
...
{
    // Convert iadixcoin:// to iadixcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("iadixcoin://"))
    {
        uri.replace(0, 12, "iadixcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "iadixcoin.lnk";
}
...
{
    return GetAutostartDir() / "iadixcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("iadixcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  iadixcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("iadixcoin-Qt"));
